// ============================================================
// UZ Bookshelf — Wood Edition
// 木目調リアルUI: 暖色系の本棚、木の質感
// ============================================================
...
function spineColorFromTitle(title) {
  // Wood edition: 暖色系の色相に限定（革・布の質感）
  const baseHues = [15, 20, 25, 30, 35, 40, 45, 180, 200, 350, 5, 10];
  const h = baseHues[stableHash(title) % baseHues.length] + (stableHash(title + 'hue') % 10 - 5);
  const s = 30 + (stableHash(title + 'sat') % 25);
  const l = 28 + (stableHash(title + 'lit') % 20);
    ...
    <div className="uz-wrap">
      <header className="uz-header">
        <div>
          ...
          <h1 className="uz-title">UZ Bookshelf — Wood Edition</h1>
        </div>
